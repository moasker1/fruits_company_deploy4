...
class Container(models.Model):
    ...
    main_commission = models.DecimalField(max_digits=10, decimal_places=2, blank=True, null=True, editable=False)
    bill_commission = models.DecimalField(max_digits=10, decimal_places=2, blank=True, null=True, editable=False)
    ...
    @property
    def bill_win(self):
        return self.bill_commission + self.carry + self.tool_rent
    ...
    def save(self, *args, **kwargs):
        if self.commission is not None:
            ...
            self.main_commission = (commission_decimal / 100) * self.total_sale_price

        # Calculate bill_commission based on total_bill_price
        self.bill_commission = (Decimal(self.commission) / 100) * self.total_bill_price
...
class RecentAction(models.Model):
    ...
    model_affected = models.CharField(max_length=50,blank=True, null=True)  
